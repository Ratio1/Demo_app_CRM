...
-- constraint says — `char_length(account_key) = 64`, the width of a SHA-256
-- hex digest — is pinned by the step file's SHA-256 in `schema_migrations`:
-- the checksum is the artefact that cannot drift, whereas `check_clause` is
-- rendered text whose spelling is the engine's business and would make this
-- file a second engine-dependent check beside the index ones.
SELECT (
