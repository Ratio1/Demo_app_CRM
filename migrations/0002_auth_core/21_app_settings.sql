-- DATA_CONTRACT.md §3.7. Two keys and no more: the key CHECK is an allowlist,
-- so a new setting is a migration step rather than a row an operator invents.
--
-- `public_origin` is the exact HTTPS origin (spec §4: stored in the database,
-- never a variable, never derived from a request header).
