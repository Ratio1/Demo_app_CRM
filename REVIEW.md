...
- **Council seats**: the three-seat, read-only planning council that produced `SIMPLIFICATION_PLAN.md` itself (all three seats Opus 5 (1M context), per the plan's own header) *did* run, before this push's implementation work. What is cut, by operator decision 4 (`SIMPLIFICATION_PLAN.md` §5 #4), is the spec's separate P4/P5/P6 implementation-review councils and the further design council — replaced by the one reviewer pass above. D-A closes as moot and the UX-architect seat's D2 substitution (`gpt-5.6-sol`/`opus`) was not exercised this push — recorded here per `BRIEF.md` in case a future push revives it.
...
> Disposition — **accepted low, not fixed.** No new gate was added this push (outside plan §4 tasks 6/11's scope; the plan permits no new tests unless a fix requires one). Re-verified in this task: `grep -rn "services\.demo\|repositories\.maintenance" Demo_app_CRM/app/routes/ Demo_app_CRM/app/main.py` on `HEAD` (`f12e159`) — zero hits; the only importers remain `app/services/demo.py:35` (imports `maintenance`) and `scripts/manage:68` (imports `demo`). Still true.
