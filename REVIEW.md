...
**App:** Demo_App_CRM — minimalistic CRM MVP. **Date:** 2026-09-22. **Commit reviewed:** `f12e159` (this file's own commit lands on top of it, on `main`).
**Reproduce:** from the checkout root that contains `_tools/pgsql`, `_tools/pgsql/pg ensure` then (once) `_tools/pgsql/pg create-app crm_test`; canonical suite from `Demo_app_CRM/`: `scripts/with-env .env.test.local -- .venv/bin/python -B -m pytest tests -p no:cacheprovider -q`; container path: `scripts/build-image`, then `docker run` with `--env-file .env.docker.test.local` on an ephemeral host port using the `--cpus=0.5 --memory=1g --memory-swap=1g --read-only --stop-timeout 25` flags of `scripts/run-local`, provisioned via `scripts/manage migrate|bootstrap|set-origin|seed-demo` using `.env.docker.test.owner.local`.

## Models actually used

- **Coordination** — Claude Fable 5.1 (authored no code and reviewed nothing itself).
- **Builder, backend and CLI** (accounts, activities, dashboard, seed/reset, and the F1 fix below) — Opus 5, on `4a5c6e6..d1a9297`, `55fa487`, `f12e159`.
- **Builder, templates and README** — Sonnet 5, on `06a171f`, `a555f3d`.
- **Test author** (test suite and this report) — Sonnet 5, on `3d8e540..9c1f19b` and this commit.
- **Reviewer** (independent, non-authoring, read-only, one pass) — Opus 5 (1M context), `claude-opus-5[1m]`, self-reported below.
- A three-model planning review (all Opus 5, 1M context) ran before implementation started; a separate multi-pass implementation review and a further design round were replaced by the single reviewer pass below, by decision of the operator.

## Independent review

> Reproduced below as delivered, mechanically unwrapped to one line per paragraph (line-wrap position is not content), except that references to internal planning documents and to decision/task identifiers have been replaced with plain wording — content and verdicts are otherwise unchanged. My own annotations, added after the fact, are set off as `> Disposition —` / `> Reconfirmed —` blockquote lines immediately following the paragraph or list item they respond to, never merged into the reviewer's sentences.

# REVIEW — Activities, dashboard, seed/reset and the container run path (independent security/privacy pass)

**Reviewer model:** Opus 5 (1M context), `claude-opus-5[1m]`. Fresh, non-authoring, read-only; no file in this repository was created or edited by this pass. One independent review, one pass.

**Scope:** `ae95ac2..9c1f19b` — 17 commits, `4a5c6e6` (conditional TLS in `scripts/start`) through `9c1f19b`, working tree clean. Image `demo-crm-app:local` id `ac7618e17e8a`, built by the builder; **not** rebuilt by this review. Every live check ran against `crm_test`; `crm` was never written.
...
**F2 · low · `tests/arch/test_gates.py:480`** — *no mechanical gate keeps the maintenance path off the web.* `maintenance` joins `_SCOPE_EXEMPT_REPOSITORY_MODULES`, so the ownership-scope gate test now permits unscoped `DELETE` statements there without asserting the boundary that makes them safe. Verified by grep: nothing under `app/routes/` imports `app.services.demo` or `app.db.repositories.maintenance` (only `app/services/demo.py:35` imports the repository, only `scripts/manage` the service), and the runtime role holds no `DELETE`. But nothing fails if a future route imports either. **Fix:** one gate asserting no module under `app/routes/`, and nothing reachable from `create_app`, imports those two modules.

> Disposition — **accepted low, not fixed.** No new gate was added this push (out of scope for this pass; new tests are added only when a fix requires one). Re-verified in this task: `grep -rn "services\.demo\|repositories\.maintenance" app/routes/ app/main.py` on `HEAD` (`f12e159`) — zero hits; the only importers remain `app/services/demo.py:35` (imports `maintenance`) and `scripts/manage:68` (imports `demo`). Still true.

**F3 · low · `README.md:40`** — *the image embeds a private TLS key and the README does not say it must never be published.* Confirmed inside the image: `/app/app/certs/dev-server.key`, `-rw-------`, owner `10001`. Intended (no mounts, by design), but the README's Build section lists only what the image lacks. **Fix:** one line — `demo-crm-app:local` is local-only because it embeds a development TLS private key; never push or share it.
...
**F4 · low · `scripts/build-image:44`** — *the `verify-full` trust anchor is a build-time copy of the local dev CA*, git-ignored, not the specification's requirement for an explicit, reviewed CA bundle for both approved database targets; no R1DB root. `.gitignore:19` still references a committed `app/certs/roots/` that does not exist. **Fix:** no code change; state it beside the R1DB row in the deferred table below.

> Disposition — **accepted, stated below** in the deferred table's R1DB row (the finding itself asks for no code change).
...
> Disposition — **accepted low, not fixed** (the fix asked only to record the exception, which this line does). Confirmed unchanged on `HEAD` via `git show --stat f12e159`: that commit touches only `scripts/manage` and `tests/concurrency/test_demo_seed_and_reset.py`, not `app/services/activities.py`.

**F6 · low · `app/db/repositories/maintenance.py:58`** — *`reset-demo` is broader than "demo rows"*: it deletes **every** row of contacts, deals, activities, receipts, sessions, `login_throttle` and `rate_budget`, so on `crm` it also removes the operator's hand-made walk records and clears throttle and rate-limit state. Maintenance-role only, `--yes` gated, `demo_reset` in the same transaction, and `users`/`app_settings`/`schema_migrations`/`audit_events` untouched. **Fix:** name `login_throttle`/`rate_budget` here so "never touches accounts/audit" is not read as "seeded rows only".
...
> Reconfirmed in this task, per check, precisely: check 1 (activities authorization) only through the two fresh canonical suite runs below, not a separate live 404/409 probe this task. Check 2 (dashboard aggregate) — live, exactly: the container's rendered tiles matched an independently computed SQL query both before and after a live mutation (Container evidence below). Check 3 (seed/reset) — live: exact before/after row counts for all seven `DEMO_DELETE_ORDER` tables plus `users`/`app_settings`/`schema_migrations`/`audit_events` (Container evidence below). Check 4 (container path) — live, but only in part: `docker inspect` (user/rootfs/limits/mounts), `find` inside the image, the response header dump, and `/health/live`→`/health/ready` did get reconfirmed; **not** reconfirmed this task: `pg_stat_ssl`, the gateway-IP hostname-mismatch probe, or the request-log secret-scan — see `not_verified`. Check 5 (scans) was **not re-run** this task; its artefacts (`sbom/`, dated 2026-09-22) are unchanged by `f12e159`. Check 6 (tests) — live: two fresh canonical suite runs, both confirmed exit 0 from the run logs, not merely the "452 passed" line.

## Security controls

- **Password policy** — F1 (medium): the CLI password path skips the blocklist and `seed-demo` removes the forced reset. Argon2id parameters, bounds, generic failures and the forced-reset gate are unchanged here.
...
- **Session cookies** — unchanged by this diff; `__Host-` cookies and rotation exercised by the shipped suite. The container's live responses confirm `no-store` on private and health responses.
- **Request validation and CSRF** — PASS: `POST /activities` goes through `start_mutation` (session → CSRF → content type → budget → forced-reset → exact body allowlist); `_CREATE_FIELDS` is the frozen form's set and any extra, repeated or non-textual part is a 400. Both `GET`s mutate nothing.
- **Injection defence** — PASS: every new statement is parameterized, scope fragments are one of exactly two literals, `ACTIVITY_FIELDS`/`ActivityFields` are the write allowlist, `?page=` is allowlisted and digit-matched, per-page clamped to 100, offset clamped. Templates autoescape; no `|safe`.
- **Security headers and network exposure** — unchanged; headers confirmed live on the container (above). Port 3000 private, published to loopback only by `run-local`.
- **Throttling and logging** — unchanged; note F6, that `reset-demo` clears `login_throttle`/`rate_budget` — maintenance role only, never reachable over HTTP. Logs carry no secrets, SQL or record bodies.
- **Audit log** — PASS: `activity_created` rides the same `SERIALIZABLE` transaction as the append; the vocabulary has no `activity_updated`/`activity_deleted` because the role has neither privilege. `demo_seeded`/`demo_reset` are written by the maintenance role with `actor_id` NULL. Deny paths are audited. The maintenance-side audit coverage (`cleanup`/`export-subject`/`erase-subject`) remains deferred.
...
**OWASP ASVS 5.0.0 L2:** *not claimed — deferred by decision of the operator.* No per-ID map exists and none is fabricated here; the controls above are implemented and covered by `tests/`.

## NOT VERIFIED (never reported as passed)

The resource gate, `RESOURCE_TESTS.md`, profile-scale seed · two replicas · image scan, hadolint, authenticated DAST, secret-scan fidelity · R1DB compatibility (BLOCKED) · hostile `PG*` env on the `scripts/manage` path (`scripts/start` scrubs; `manage` was not probed) · invalid-CA and wrong-hostname behaviour beyond the one live hostname-mismatch probe recorded above. The `docker stats` reading taken during this review — **49.88 MiB / 1 GiB, CPU 0.22%** — is an **idle** snapshot of the capped container and is not the resource gate.
...
*(End of the reviewer's report, reproduced above with only planning-reference wording changed.)*

## Test evidence (this task, Sonnet 5)
...
| Requirement | Status | What is actually true |
|---|---|---|
| Required resource gate | **NOT VERIFIED — deferred by operator** | No 20-minute run, no p95 figure, no peak-memory number at profile scale. The container is genuinely run capped at 0.5 core / 1 GiB and a `docker stats` snapshot is recorded. |
| Two replicas | **NOT VERIFIED — deferred** | One replica by design; no shared-session/throttle-across-replicas claim is made. |
| Deployment hardening (invalid CA, wrong hostname, hostile `PG*` env) | **NOT VERIFIED** | `app/config.py` passes every libpq parameter explicitly; one unit test pins `sslmode=verify-full` + explicit `sslrootcert`. |
| `manage cleanup`/`export-subject`/`erase-subject`; maintenance-side audit coverage | **NOT IMPLEMENTED — deferred** | No retention pruning, no subject export, no erasure path. |
| ASVS mapping | **NOT CLAIMED — deferred** | No per-ID map. The controls above are implemented and covered by `tests/security`. |
| Administrator MFA | **Recorded gap** | Administrator MFA absent; required before real-data use. |
| Quality scans (image scan, hadolint, DAST, secret-scan fidelity) | **NOT VERIFIED** | Tools unavailable here; `pip-audit`, `bandit` and one SBOM run once, dated, with raw output committed. |
| Review process | **PARTIAL — reduced by operator** | One fresh reviewer, one pass, instead of a multi-reviewer round with a re-review pass; no further design round. The models used in the planning review that ran before implementation are recorded above. |
| R1DB leg | **BLOCKED** | No endpoint, fork/version or credentials; no compatibility claim is made. |
| WAR source-build, `war-*.sh`, tunnel | **Out of scope per operator, 2026-09-21** | Never reported as passed. |
| Privacy fence | **Binding** | This build is for fictional `example.test` data only and must not hold real personal data until subject export, erasure, retention pruning and verified administrator MFA exist. |

This task adds no new deferral beyond the table above. F2/F3/F5's "accepted low, not fixed" dispositions are recorded next to each finding above, not as new table rows; F4's is folded into the R1DB row: the shipped `verify-full` trust anchor is the local dev CA only, not an R1DB-reviewed bundle.
