#!/usr/bin/env bash
#
# dev-run.sh — serve Demo_App_CRM on the host, over TLS, for a human
# (contracts/slice-a.md §1.2, delta D8). It is NOT the container entrypoint:
# that is scripts/start, which binds 0.0.0.0:3000 and adds no TLS of its own.
#
#   scripts/dev-run.sh
#
# What it does, in order:
#
#   1. Scrubs every PG* variable out of the environment (delta D9, ruling
#      R35). The application passes every libpq parameter explicitly, so a
#      hostile PGSSLMODE or PGHOST cannot *override* anything — but a
#      PGSERVICE naming a service that does not exist is a hard connection
#      failure, which is an availability lever any process that can set this
#      process's environment could pull. Scrubbing is fail-closed and costs
#      nothing.
...
# relative to the app package itself (delta D2), but the uvicorn --ssl-* paths
# below and `app.main:app`'s import both still need the root as the cwd.
script_dir="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
root_dir="$(cd -- "${script_dir}/.." && pwd)"
cd -- "$root_dir"

# 1. D9 / R35 — every PG* name, unconditionally, before anything else runs.
...
# 3. Same flag list as scripts/start (D8), plus the host/port/TLS trio.
