"""Pre-auth session cleanup deletes up to 100 expired rows on every pre-auth insert.
...
clock past that TTL cannot accumulate expired rows at all, because the
cleanup fires on every single one of those inserts and sweeps the
...
async def test_a_login_page_fetch_reclaims_up_to_100_expired_preauth_rows(
  ...
  claim: (1) the total pre-auth row count drops by exactly
  ``100 - 1`` (100 reclaimed, 1 new row inserted) rather than "some";
  (2) a **live** (not-yet-expired) pre-auth row survives untouched — cleanup
  ...
  # seeded live row (reuse only applies to a cookie the client actually
  # presents).
