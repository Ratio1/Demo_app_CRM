...
The runtime role holds ``INSERT`` and ``SELECT`` on this table and no
...
leaves neither a session row nor a claim that a login happened.
"""
...
async def insert_event(
  ...
  *,
  ...
  object_id : UUID | None
    The object acted on, or ``None`` where the action names no object.
  action : str
  ...
  correlation_id : str
   ...
   , which ``ck_audit_events_correlation`` enforces exactly. It is
